...
(() => {
  ...
  var _harnessPlugins = [];
  var _callbackNames = "";
  var _harness = function _harness2(targets2) {
    var target = targets2[0], harnessPlugin, i;
    _isObject(target) || _isFunction(target) || (targets2 = [targets2]);
    ...
    i = targets2.length;
    while (i--) {
      targets2[i] && (targets2[i]._gsap || (targets2[i]._gsap = new GSCache(targets2[i], harnessPlugin))) || targets2.splice(i, 1);
    }
    return targets2;
  };
  ...
  var _renderZeroDurationTween = function _renderZeroDurationTween2(tween, totalTime, suppressEvents, force) {
    var prevRatio = tween.ratio, ratio = totalTime < 0 || !totalTime && (!tween._start && _parentPlayheadIsBeforeStart(tween) && !(!tween._initted && _isFromOrFromStart(tween)) || (tween._ts < 0 || tween._dp._ts < 0) && !_isFromOrFromStart(tween)) ? 0 : 1, repeatDelay2 = tween._rDelay, tTime = 0, pt, iteration, prevIteration;
    if (repeatDelay2 && tween._repeat) {
      tTime = _clamp(0, tween._tDur, totalTime);
      iteration = _animationCycle(tTime, repeatDelay2);
      tween._yoyo && iteration & 1 && (ratio = 1 - ratio);
      if (iteration !== _animationCycle(tween._tTime, repeatDelay2)) {
        prevRatio = 1 - ratio;
  ...
  var Animation = /* @__PURE__ */ function() {
    ...
    _proto.repeatDelay = function repeatDelay2(value) {
      if (arguments.length) {
        var time = this._time;
        this._rDelay = value;
    ...
    _proto.pause = function pause2(atTime, suppressEvents) {
  ...
  var Timeline = /* @__PURE__ */ function(_Animation) {
    ...
    _proto2.to = function to(targets2, vars, position) {
      _createTweenType(0, arguments, this);
      return this;
    };
    _proto2.from = function from(targets2, vars, position) {
      _createTweenType(1, arguments, this);
      return this;
    };
    _proto2.fromTo = function fromTo(targets2, fromVars, toVars, position) {
      _createTweenType(2, arguments, this);
      return this;
    };
    _proto2.set = function set2(targets2, vars, position) {
      vars.duration = 0;
      vars.parent = this;
      _inheritDefaults(vars).repeatDelay || (vars.repeat = 0);
      vars.immediateRender = !!vars.immediateRender;
      new Tween(targets2, vars, _parsePosition(this, position), 1);
      return this;
    };
    _proto2.call = function call(callback, params, position) {
      return _addToTimeline(this, Tween.delayedCall(0, callback, params), position);
    };
    _proto2.staggerTo = function staggerTo(targets2, duration2, vars, stagger2, position, onCompleteAll, onCompleteAllParams) {
      vars.duration = duration2;
      vars.stagger = vars.stagger || stagger2;
      vars.onComplete = onCompleteAll;
      vars.onCompleteParams = onCompleteAllParams;
      vars.parent = this;
      new Tween(targets2, vars, _parsePosition(this, position));
      return this;
    };
    _proto2.staggerFrom = function staggerFrom(targets2, duration2, vars, stagger2, position, onCompleteAll, onCompleteAllParams) {
      vars.runBackwards = 1;
      _inheritDefaults(vars).immediateRender = _isNotFalse(vars.immediateRender);
      return this.staggerTo(targets2, duration2, vars, stagger2, position, onCompleteAll, onCompleteAllParams);
    };
    _proto2.staggerFromTo = function staggerFromTo(targets2, duration2, fromVars, toVars, stagger2, position, onCompleteAll, onCompleteAllParams) {
      toVars.startAt = fromVars;
      _inheritDefaults(toVars).immediateRender = _isNotFalse(toVars.immediateRender);
      return this.staggerTo(targets2, duration2, toVars, stagger2, position, onCompleteAll, onCompleteAllParams);
    };
    ...
    _proto2.killTweensOf = function killTweensOf(targets2, props, onlyActive) {
      var tweens = this.getTweensOf(targets2, onlyActive), i = tweens.length;
      while (i--) {
        _overwritingTween !== tweens[i] && tweens[i].kill(targets2, props);
      }
      return this;
    };
    _proto2.getTweensOf = function getTweensOf2(targets2, onlyActive) {
      var a = [], parsedTargets = toArray(targets2), child = this._first, isGlobalTime = _isNumber(onlyActive), children;
      while (child) {
        if (child instanceof Tween) {
    ...
    _proto2.tweenTo = function tweenTo(position, vars) {
      ...
      var tl2 = this, endTime = _parsePosition(tl2, position), _vars = vars, startAt = _vars.startAt, _onStart = _vars.onStart, onStartParams = _vars.onStartParams, immediateRender = _vars.immediateRender, initted, tween = Tween.to(tl2, _setDefaults({
        ...
        onStart: function onStart() {
          tl2.pause();
          if (!initted) {
            var duration2 = vars.duration || Math.abs((endTime - (startAt && "time" in startAt ? startAt.time : tl2._time)) / tl2.timeScale());
  ...
  var _addPropTween = function _addPropTween2(target, prop, start, end, index, targets2, modifier, stringFilter, funcParam, optional) {
    _isFunction(end) && (end = end(index || 0, target, targets2));
  ...
  var _processVars = function _processVars2(vars, index, target, targets2, tween) {
    _isFunction(vars) && (vars = _parseFuncOrString(vars, tween, index, target, targets2));
    if (!_isObject(vars) || vars.style && vars.nodeType || _isArray(vars) || _isTypedArray(vars)) {
      return _isString(vars) ? _parseFuncOrString(vars, tween, index, target, targets2) : vars;
    }
    var copy = {}, p;
    for (p in vars) {
      copy[p] = _parseFuncOrString(vars[p], tween, index, target, targets2);
    }
    return copy;
  };
  var _checkPlugin = function _checkPlugin2(property, vars, tween, index, target, targets2) {
    var plugin, pt, ptLookup, i;
    if (_plugins[property] && (plugin = new _plugins[property]()).init(target, plugin.rawVars ? vars[property] : _processVars(vars[property], index, target, targets2, tween), tween, index, targets2) !== false) {
  ...
  var _initTween = function _initTween2(tween, time, tTime) {
    var vars = tween.vars, ease = vars.ease, startAt = vars.startAt, immediateRender = vars.immediateRender, lazy = vars.lazy, onUpdate = vars.onUpdate, runBackwards = vars.runBackwards, yoyoEase = vars.yoyoEase, keyframes = vars.keyframes, autoRevert = vars.autoRevert, dur = tween._dur, prevStartAt = tween._startAt, targets2 = tween._targets, parent = tween.parent, fullTargets = parent && parent.data === "nested" ? parent.vars.targets : targets2, autoOverwrite = tween._overwrite === "auto" && !_suppressOverwrites, tl2 = tween.timeline, cleanVars, i, p, pt, target, hasPriority, gsData, harness, plugin, ptLookup, index, harnessVars, overwritten;
    ...
    if (!tl2 || keyframes && !vars.stagger) {
      harness = targets2[0] ? _getCache(targets2[0]).harness : 0;
      ...
      if (startAt) {
        _removeFromParent(tween._startAt = Tween.set(targets2, _setDefaults({
          data: "isStart",
          overwrite: false,
          parent,
        ...
        if (!prevStartAt) {
          ...
          _removeFromParent(tween._startAt = Tween.set(targets2, p));
          tween._startAt._dp = 0;
          tween._startAt._sat = tween;
      ...
      for (i = 0; i < targets2.length; i++) {
        target = targets2[i];
        gsData = target._gsap || _harness(targets2)[i]._gsap;
        tween._ptLookup[i] = ptLookup = {};
        _lazyLookup[gsData.id] && _lazyTweens.length && _lazyRender();
        index = fullTargets === targets2 ? i : fullTargets.indexOf(target);
  ...
  var _addAliasesToVars = function _addAliasesToVars2(targets2, vars) {
    var harness = targets2[0] ? _getCache(targets2[0]).harness : 0, propertyAliases = harness && harness.aliases, copy, p, i, aliases;
    if (!propertyAliases) {
      return vars;
    }
    copy = _merge({}, vars);
  ...
  var _parseFuncOrString = function _parseFuncOrString2(value, tween, i, target, targets2) {
    return _isFunction(value) ? value.call(tween, i, target, targets2) : _isString(value) && ~value.indexOf("random(") ? _replaceRandom(value) : value;
  };
  ...
  var Tween = /* @__PURE__ */ function(_Animation2) {
    ...
    function Tween2(targets2, vars, position, skipInherit) {
      var _this3;
      ...
      if (keyframes || stagger2 || _isFuncOrString(duration2) || _isFuncOrString(delay)) {
        ...
        if (stagger2 || _isFuncOrString(duration2) || _isFuncOrString(delay)) {
          ...
          tl2.duration() ? duration2 = delay = 0 : _this3.timeline = 0;
        } else if (keyframes) {
          ...
          if (_isArray(keyframes)) {
            ...
            tl2.duration() < duration2 && tl2.to({}, {
              duration: duration2 - tl2.duration()
            });
          }
        }
        duration2 || _this3.duration(duration2 = tl2.duration());
      } else {
        _this3.timeline = 0;
      }
    ...
    _proto3.targets = function targets2() {
    ...
    _proto3.kill = function kill(targets2, vars) {
      if (vars === void 0) {
        vars = "all";
      }
      if (!targets2 && (!vars || vars === "all")) {
      ...
      if (this.timeline) {
        ...
        this.timeline.killTweensOf(targets2, vars, _overwritingTween && _overwritingTween.vars.overwrite !== true)._first || _interrupt(this);
        this.parent && tDur !== this.timeline.totalDuration() && _setDuration(this, this._dur * this.timeline._tDur / tDur, 0, 1);
        return this;
      }
      var parsedTargets = this._targets, killingTargets = targets2 ? toArray(targets2) : parsedTargets, propTweenLookup = this._ptLookup, firstPT = this._pt, overwrittenProps, curLookup, curOverwriteProps, props, p, pt, i;
    ...
    Tween2.to = function to(targets2, vars) {
      return new Tween2(targets2, vars, arguments[2]);
    };
    Tween2.from = function from(targets2, vars) {
    ...
    Tween2.fromTo = function fromTo(targets2, fromVars, toVars) {
      return _createTweenType(2, arguments);
    };
    Tween2.set = function set2(targets2, vars) {
      vars.duration = 0;
      vars.repeatDelay || (vars.repeat = 0);
      return new Tween2(targets2, vars);
    };
    Tween2.killTweensOf = function killTweensOf(targets2, props, onlyActive) {
      return _globalTimeline.killTweensOf(targets2, props, onlyActive);
    };
    return Tween2;
  }(Animation);
  ...
  var _gsap = {
    ...
    getTweensOf: function getTweensOf(targets2, onlyActive) {
      return _globalTimeline.getTweensOf(targets2, onlyActive);
    },
    ...
    isTweening: function isTweening(targets2) {
      return _globalTimeline.getTweensOf(targets2, true).length > 0;
    },
    ...
    registerEffect: function registerEffect(_ref3) {
      ...
      _effects[name] = function(targets2, vars, tl2) {
        return effect(toArray(targets2), _setDefaults(vars || {}, defaults3), tl2);
      };
      if (extendTimeline) {
        Timeline.prototype[name] = function(targets2, vars, position) {
          return this.add(_effects[name](targets2, _isObject(vars) ? vars : (position = vars) && {}, this), position);
        };
      }
    },
  ...
  var _addModifiers = function _addModifiers2(tween, modifiers) {
    var targets2 = tween._targets, p, i, pt;
    for (p in modifiers) {
      i = targets2.length;
      while (i--) {
        ...
        if (pt && (pt = pt.d)) {
          ...
          pt && pt.modifier && pt.modifier(modifiers[p], tween, targets2[i], p);
        }
      }
    }
  };
  ...
  var gsap = _gsap.registerPlugin({
    name: "attr",
    init: function init(target, vars, tween, index, targets2) {
      var p, pt, v;
      this.tween = tween;
      for (p in vars) {
        v = target.getAttribute(p) || "";
        pt = this.add(target, "setAttribute", (v || 0) + "", vars[p], index, targets2, 0, 0, p);
        pt.op = p;
        pt.b = v;
        this._props.push(p);
      }
  ...
  var CSSPlugin = {
    ...
    init: function init3(target, vars, tween, index, targets2) {
      ...
      for (p in vars) {
        ...
        endValue = vars[p];
        if (_plugins[p] && _checkPlugin(p, vars, tween, index, target, targets2)) {
          continue;
        }
        type = typeof endValue;
        specialProp = _specialProps[p];
        if (type === "function") {
          endValue = endValue.call(tween, index, target, targets2);
          type = typeof endValue;
        }
        ...
        if (specialProp) {
          ...
          this.add(style, "setProperty", startValue, endValue, index, targets2, 0, 0, p);
          props.push(p);
          inlineProps.push(p, 0, style[p]);
        } else if (type !== "undefined") {
          if (startAt && p in startAt) {
            startValue = typeof startAt[p] === "function" ? startAt[p].call(tween, index, target, targets2) : startAt[p];
          ...
          if (isTransformRelated || (endNum || endNum === 0) && (startNum || startNum === 0) && !_complexExp.test(endValue) && p in style) {
            ...
            if (p in target) {
              this.add(target, p, startValue || target[p], relative ? relative + endValue : endValue, index, targets2);
  ...
  var _endAnimation = function _endAnimation2(animation2, reversed, pause2) {
    return animation2 && animation2.progress(reversed ? 0 : 1) && pause2 && animation2.pause();
  };
  ...
  var _getClosestLabel = function _getClosestLabel2(animation2) {
    return function(value) {
      return gsap3.utils.snap(_getLabelRatioArray(animation2), value);
    };
  };
  ...
  var ScrollTrigger2 = /* @__PURE__ */ function() {
    function ScrollTrigger3(vars, animation2) {
    ...
    _proto.init = function init5(vars, animation2) {
      ...
      self.scrubDuration = function(value) {
        ...
        if (!scrubSmooth) {
          ...
          scrubTween ? scrubTween.duration(value) : scrubTween = gsap3.to(animation2, {
            ease: "expo",
            totalProgress: "+=0",
            inherit: false,
      ...
      if (animation2) {
        ...
        animation2._initted && !self.isReverted || animation2.vars.immediateRender !== false && vars.immediateRender !== false && animation2.duration() && animation2.render(0, true, true);
      ...
      if (snap3) {
        ...
        snapFunc = _isFunction3(snap3.snapTo) ? snap3.snapTo : snap3.snapTo === "labels" ? _getClosestLabel(animation2) : snap3.snapTo === "labelsDirectional" ? _getLabelAtDirection(animation2) : snap3.directional !== false ? function(value, st) {
            ...
            if (scroll <= end && scroll >= start && endScroll !== scroll) {
              ...
              tweenTo(endScroll, {
                ...
                onComplete: function onComplete() {
                  self.update();
                  lastSnap = scrollFunc();
                  if (animation2 && !isToggle) {
                    scrubTween ? scrubTween.resetTo("totalProgress", endValue, animation2._tTime / animation2._tDur) : animation2.progress(endValue);
                  }
      ...
      self.refresh = function(soft, force, position, pinOffset) {
        ...
        if (invalidateOnRefresh && animation2) {
          animation2.revert({
            kill: false
          }).invalidate();
          animation2.getChildren && animation2.getChildren(true, true, false).forEach(function(t) {
        ...
        _refreshing = 0;
        animation2 && isToggle && (animation2._initted || prevAnimProgress) && animation2.progress() !== prevAnimProgress && animation2.progress(prevAnimProgress || 0, true).render(animation2.time(), true, true);
        if (isFirstRefresh || prevProgress !== self.progress || containerAnimation || invalidateOnRefresh || animation2 && !animation2._initted) {
          animation2 && !isToggle && (animation2._initted || prevProgress || animation2.vars.immediateRender !== false) && animation2.totalProgress(containerAnimation && start < -1e-3 && !prevProgress ? gsap3.utils.normalize(start, end, 0) : prevProgress, true);
      ...
      self.update = function(reset, recordVelocity, forceFake) {
        ...
        if (clipped !== prevProgress2 && self.enabled) {
          ...
          preventOverlaps && (toggled || isTakingAction) && (isTakingAction || scrub || !animation2) && (_isFunction3(preventOverlaps) ? preventOverlaps(self) : self.getTrailing(preventOverlaps).forEach(function(t) {
          ...
          if (!isToggle) {
            if (scrubTween && !_refreshing && !_startup2) {
              ...
              animation2.totalProgress(clipped, !!(_refreshing && (lastRefresh || reset)));
            }
          }
          if (pin) {
      ...
      if (animation2 && animation2.add && !change) {
  ...
  ScrollTrigger2.saveStyles = function(targets2) {
    return targets2 ? _toArray(targets2).forEach(function(target) {
  ...
  ScrollTrigger2.batch = function(targets2, vars) {
    ...
    _toArray(targets2).forEach(function(target) {
      var config3 = {};
      for (p in varsCopy) {
  ...
  function splitWordsAndChars(textNode, settings) {
    ...
    words2 = toWords(VALUE).reduce(function(result, WORD, idx, arr) {
      var wordElement;
  ...
  function split(node, settings) {
    ...
    return childNodes.reduce(function(result, child) {
      var _split = split(child, settings), words2 = _split.words, chars = _split.chars;
      return {
        words: [].concat(_toConsumableArray(result.words), _toConsumableArray(words2)),
    ...
    _createClass2(SplitType2, [{
      ...
      value: function split$1(options) {
        ...
        this.elements.forEach(function(element) {
          ...
          var _split2 = split(element, _this.settings), words2 = _split2.words, chars = _split2.chars;
          _this.words = [].concat(_toConsumableArray(_this.words), _toConsumableArray(words2));
  ...
  // src/js/animations/fadeTextFromBottom.js
  ...
  function initializeAnimations() {
    ...
    fadeFromLeftElements.forEach((element) => {
      let targets2 = element;
      if (element.tagName.toLowerCase() === "h1") {
        targets2 = element.children;
      }
      if (element.hasAttribute("split-lines")) {
        ...
        targets2 = splitTypeInstance.lines;
      }
      const animateReverse = element.hasAttribute("animate-reverse");
      gsapWithCSS.to(targets2, {
        opacity: 1,
        y: 0,
        duration: 1,
        stagger: targets2 instanceof NodeList || Array.isArray(targets2) ? 0.2 : 0,
        // Apply stagger only if it's a collection of elements
        ease: "back.inOut",
        scrollTrigger: {
          trigger: element,
          start: "top 90%",
          once: !animateReverse,
          // Play only once unless animate-reverse is true
          invalidateOnRefresh: true,
          // Recalculate positions on screen resize
  ...
  // src/js/animations/imageParallax.js
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  ...
  _getGSAP5() && gsap4.registerPlugin(TextPlugin);

  // src/js/pages/aboutUs.js
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  gsapWithCSS.registerPlugin(TextPlugin);
  var letters = document.querySelectorAll(".logo-letter");
  var stickyElement = document.querySelector(".about-hero_sticky");
  var stickyLogo = document.querySelector(".about-hero_logo");
  var introText = document.querySelector(".about-hero_sticky-intro");
  var positions = [
    { x: -50, y: -50 },
    // Position for the first letter
    { x: -150, y: -100 },
    // Position for the second letter
    { x: 0, y: -150 },
    // Position for the third letter
    { x: 250, y: -120 },
    // Position for the fourth letter
    { x: 100, y: 50 },
    // Position for the fifth letter
    { x: -20, y: 200 },
    // Position for the sixth letter
    { x: -100, y: 100 }
    // Position for the seventh letter
  ];
  letters.forEach((letter, index) => {
    ...
    gsapWithCSS.fromTo(
      letter,
      {
        ...
        // Start from the specified Y position
        x: x * window.innerWidth / 100,
        // Start from the specified X position
        scale: 6,
        // Start with each letter scaled up
        opacity: 0
        // Start with each letter invisible
      },
      {
        y: 0,
        // Move the letter to its original position
        x: 0,
        // Move the letter to its original X position
        scale: 1,
        ...
        scrollTrigger: {
          trigger: ".about-hero",
          // Trigger animation when .about-hero_trigger is in view
          start: `center ${90 - index * 5}%`,
          // Dynamic start value based on index
          end: "center 50%",
          // End when the top of .about-hero_trigger is 50% from the top of the viewport
          scrub: 1
          // Scrub smoothly with a duration of 1 second
    ...
    gsapWithCSS.fromTo(
      ...
      },
      {
        ...
        scrollTrigger: {
          trigger: ".about-hero",
          // Trigger the animation when stickyElement is in view
          start: "center 90%",
          end: "center 80%",
          scrub: 1
        }
    ...
    gsapWithCSS.fromTo(
      ...
      },
      {
        ...
        // Change this value to your desired max-width
        maxHeight: "0px",
        // Change this value to your desired max-height
        ease: "power2.inOut",
        // Easing function (optional)
        scrollTrigger: {
          trigger: ".about-hero",
          // Trigger the animation when stickyElement is in view
          start: "center 40%",
          // Start the animation at 40% scroll progress
          end: "bottom 25%",
          // End the animation at 25% scroll progress
          scrub: 1
          // Smoothly animate with scrolling
          // markers: true, // Enable markers to visualize trigger points
        }
      }
    );
    gsapWithCSS.fromTo(
      ...
      },
      {
        ...
        scrollTrigger: {
          ...
          // End the animation at 30% scroll progress
          scrub: 1
          // Smoothly animate with scrolling
          // markers: true, // Enable markers to visualize trigger points
        }
    ...
    gsapWithCSS.fromTo(
      ...
      },
      {
        ...
        scrollTrigger: {
          ...
          // End the animation at 10% scroll progress
          scrub: 1
          // Smoothly animate with scrolling
          // markers: true, // Enable markers to visualize trigger points
        }
      ...
      paths.forEach((path) => {
        ...
        gsapWithCSS.fromTo(
          path,
          {
            ...
            // Set the strokeDasharray to the length of the path
            strokeDashoffset: fillDirection * pathLength
            // Start with the stroke hidden, reverse for right to left
          },
          {
            strokeDashoffset: 0,
            // Fill the stroke by reducing dashoffset to 0
            scrollTrigger: {
              trigger: path,
              // Trigger the animation when the path is in view
              start: "top 60%",
              // Start the animation when the top of the SVG is 60% from the top of the viewport
              end: "top 20%",
              // End the animation when the top of the SVG is 20% from the top
              scrub: true
              // Smoothly animate in sync with the scroll position
            }
          }
        );
      });
    ...
    elements.forEach((element) => {
      const points = Array.from({ length: numPoints }, () => {
        return {
          ...
          // Random x between -radius and radius
          y: Math.random() * 2 * radius - radius
          // Random y between -radius and radius
        };
      });
      const duration2 = Math.random() * (maxDuration - minDuration) + minDuration;
      const tl2 = gsapWithCSS.timeline({ repeat: -1, yoyo: true });
      points.forEach((point, index) => {
        tl2.to(element, {
          ...
          ease: "power1.inOut",
          delay: index === 0 ? 0 : 1
          // Ensure there is no delay on the first point
        });
      });
      tl2.to(element, {
        x: points[0].x,
  ...
  var words = gsapWithCSS.utils.toArray(".type-del");
  var blinking = gsapWithCSS.timeline({
    repeat: -1,
    paused: true
  });
  ...
  var animation = gsapWithCSS.timeline({ repeat: 20, paused: true });
  var targets = document.querySelectorAll(".type-del");
